exports.getCategoryViewModel = function(categories) {
	...
	for (var i = 0; i < categories.length; i++) {
		var model = {}
		  ...
		  , startDate = new Date(category.starts)
		  , today = new Date();
		...
		if(startDate > today) { 
			...
			if (!!category.latestGame.homeScore) {
				...
				if (category.latestGame.homeScore !== category.latestGame.awayScore) {
					model.image = category.latestGame.winner.toLowerCase()
				} else {
					model.image = category.latestGame.home.toLowerCase() + category.latestGame.away.toLowerCase()
				}
			} else {
